...
class UpdateDataCenterDeviceIOTags(APIView):

    def post(self, request):
        data = request.body.decode("utf-8")
        requestData = json.loads(data)

        response = ConfigDevicesIOTags().custom_update_com_io_tags(requestData=requestData)
        if response == 'success':
            return HttpResponse(response, "application/json")
        else:
            return HttpResponseBadRequest(response)
...
class ReadDeviceIoTProperties(APIView):

    def get(self, request):
        jsonData: Edge = config.read_setting()
        io_tags: List[IOTag] = list(jsonData.edgedevice.DataCenter.COM1.devices[0].IOTags)
        io_tag_list = []
        for sensors in io_tags:
            io_tag = sensors.to_dict()
            fields = {
                "devicename": io_tag["Name"],
                "address": io_tag["Address"],
                "initialvalue": io_tag["Initial Value"],
                "description": io_tag["Description"]
            }
            io_tag_list.append(fields)
        return_data = json.dumps(io_tag_list, indent=4)
